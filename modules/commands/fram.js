...
module.exports.run = async function ({ event, api, args }) {    
    ...
    if (!mention[0]) return api.sendMessage("ᴘʟᴇᴀꜱᴇ ᴍᴇɴᴛɪᴏɴ 1 ᴘᴇʀꜱᴏɴ.", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "ᴍᴀᴅᴇ ʙʏ ɴᴏʙɪᴛᴀ-ʙᴏᴛ🤖❤️", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
                                }
